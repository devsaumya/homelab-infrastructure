...
Authoritative structure and conventions for all Infrastructure as Code (IaC) in the `homelab-infrastructure` repository. [conversation_history:0]

---

## Goals

- All infra changes are declarative, reproducible, and versioned. [conversation_history:0]
- Core tools: [conversation_history:0]
  - Terraform for Cloudflare/DNS and external integrations. [conversation_history:0]
  - Ansible for VM configuration and OS-level setup. [conversation_history:0]
  - Kubernetes manifests (with Kustomize) for k3s workloads. [conversation_history:0]
  - Docker Compose for VM2 services (monitoring, security, utilities). [conversation_history:0]
...
**Conventions**: [conversation_history:0]  
...
2. Bootstrap VMs with Ansible (base OS hardening, packages, Docker, k3s). [conversation_history:0]  
3. Provision external infra with Terraform (Cloudflare DNS, Cloudflare Tunnel, WAF/Zero Trust, health checks). [conversation_history:0]  
4. Apply Kubernetes base + overlays to k3s. [conversation_history:0]  
5. Bring up Docker stacks on VM2 (monitoring, services, security). [conversation_history:0]  

This file is the single source of truth for IaC structure; extend it instead of adding ad-hoc layouts when you grow the lab. [conversation_history:0]
